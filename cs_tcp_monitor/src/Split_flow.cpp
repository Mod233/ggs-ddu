...
int dns_ip(char*dir, std::string result) {
	...
	while (true) {
		...
		if (dport != uint16_t(53) && sport != uint16_t(53))
			continue;
		if (hdr.caplen - (sizeof(ether_header) + (ipptr->ihl) * 4 + 8) == 0)
			continue;
		...
		if (show[name] % 300 != 0) {
			char filename[100];
			sprintf(filename, "%s/%s-%s-%d.pcap", filedir.c_str(),
					min(sip, dip).c_str(), max(sip, dip).c_str(),
					show[name] / 300);
...
int flow_ip(char*dir, std::string result) {
	char errbuf[PCAP_ERRBUF_SIZE];
	pcap_t* descr;
	const u_char *packet;
	struct pcap_pkthdr hdr; /* pcap.h */
	...
	while (true) {
		...
		if (dport == uint16_t(53) || sport == uint16_t(53)
				|| dport == uint16_t(443) || sport == uint16_t(443))
			continue;
		std::string sip = inet_ntoa(srcip);
		std::string dip = inet_ntoa(dstip);
		std::string name;
...
#define NOACK 0
int judge_pcap(char*dir, std::string result) {
	char errbuf[PCAP_ERRBUF_SIZE];
	pcap_t* descr;
	const u_char *packet;
	struct pcap_pkthdr hdr; /* pcap.h */
	...
	while (true) {
...
#if(NOACK)
		if (ntohs(ipptr->tot_len) - (ipptr->ihl) * 4 - (tcpptr->th_off) * 4 > 0)
		...
		if (show.count(name))
			...
			show[name] = 0;
		std::string filedir = result + "/small_pcap/" + name;
		int flag = mkdir(filedir.c_str(), 0777);
		// 	if(flag!=0) printf("mkdir file %s failed\n", filedir.c_str());
		if (show[name] % 300 != 0) {
			char filename[100];
...
#endif
			std::string result =
					"/home/csober/Documents/Github/ggs-ddu/cs_tcp_monitor/src/Debug-Pcap";
//			dns_ip(childpath, result);
//			flow_ip(childpath, result);
			judge_pcap(childpath, result);
