...
//	if(delta_time_first < 0.95) return 0;
	...
	for(int i=0;i<slice_num;i++){
		...
		if(clu[i].pkt_num > 3){
			...
			if(clu[i].pkt_tag[pos2] || clu[i].pkt_tag[pos3]) {outctl_num++; printf("slice num is %d pkt_num is %d\n", i,clu[i].pkt_num);}
		else if(clu[i].pkt_num >1)
			if(clu[i].pkt_tag[pos2]) {outctl_num++; printf("slice num is %d pkt_num is %d\n", i,clu[i].pkt_num);}
		}
	}
#if(SHOW_OUTCTL)
