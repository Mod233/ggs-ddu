...
#include <sys/stat.h>
#include <pcap.h>
unsigned int subnet_intranet = ntohl(inet_addr("37.0.0.0"));      //存储子网ip，用于区分内部IP地址和外部IP地址
unsigned int subnet_extranet = ntohl(inet_addr("0.0.0.0"));     //存储子网ip，用于区分内部IP地址和外部IP地址
...
//    		printf("Finish reading!\n");
    	...
    	dip = inet_ntoa(dstip);
    	tag = 0;
    	int tcplen = ipptr->tot_len - sizeof(iphdr) - sizeof(tcpptr);
