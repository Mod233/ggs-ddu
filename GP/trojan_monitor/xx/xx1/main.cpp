...
map<int, string> flow_name;

winpcap my_pcap;        //抓包类

char cap[] = "/root/4/1/4.pcap";

//char cap[] = "/home/fullname/16mm/cmdrat.pcap"; n
//char cap[] = "/home/fullname/16mm/darkmoon.pcap"; y
//char cap[] = "/home/fullname/16mm/fbi.pcap";
//char cap[] = "/home/fullname/16mm/gh0st.pcap";
//char cap[] = "/home/fullname/16mm/ghost10.pcap";
//char cap[] = "/home/fullname/16mm/hongheirat.pcap";
//char cap[] = "/home/fullname/16mm/ratflooder.pcap";
//char cap[] = "/home/fullname/16mm/remoteghost.pcap";
//char cap[] = "/home/fullname/16mm/ruser.pcap";
//char cap[] = "/home/fullname/16mm/shuang11.pcap";
//char cap[] = "/home/fullname/16mm/spy-net.pcap";
//char cap[] = "/home/fullname/16mm/star.pcap";
//char cap[] = "/home/fullname/16mm/xdoor.pcap";
//char cap[] = "/home/fullname/16mm/ximenrat.pcap";
//char cap[] = "/home/fullname/16mm/yiyuyan.pcap";
//char cap[] = "/home/fullname/16mm/yuanxiaojie.pcap";

//char cap[] = "/home/fullname/16mm/1.pcap";

//char cap[] = "/root/16mm/cmdrat.pcap";
//char cap[] = "/home/fullname/16mm/darkmoon.pcap";
//char cap[] = "/home/fullname/16mm/gh0st.pcap";
//char cap[] = "/home/fullname/16mm/spy-net.pcap";
//char cap[] = "/home/fullname/16mm/xdoor.pcap";
...
int packet_capture()
{
	...
	int p_num;
	//unsigned short dstport;
	subnet = ip_chartoint(ipa);    //src & subnet_mask == subnet: up
	...
	while ((ret = pcap_next_ex(my_pcap.adhandle, &header, &pkt_data)) >= 0)
	{
		...
		/*if(pn<10)
			printf("dstport:%x, IPhdrlen:%d, iplen:%d, seq:%u, caplen:%d, len:%d\n",ntohs(pTCPHdr->dest_port),IPhdrlen,ntohs(pIPHdr->ip_total_length),ntohl(pTCPHdr->sequence),header->caplen,header->len);
		else
			return 0;*/
	 ...
	 //若不存在，且标志位满足SYN=1 ACK=0 RST=0（连接请求，第一次握手）
		...
		if(flow_ptr->alarm == 1)
		{
			...
			/*write(ff, (char *)header, 4);
			write(ff, ((char *)header)+8, 4);
			write(ff, ((char *)header)+16, sizeof(*header)-16);
			write(ff, pkt_data, header->caplen);*/
			close(ff);
		}
		...
		/*int cc=0;
		for(int i=0;i<flow_ptr->packet_number;i++)
			if(flow_ptr->sequence[i]==ntohl(pTCPHdr->sequence))
			{
				cc=1;
				break;
			}
		...
		/*if(pn<25)
			printf("%d: %f, %d, %d, %d\n",pn,(double(header->ts.tv_sec) + double(header->ts.tv_usec)/1000000.0),(ntohs(pIPHdr->ip_total_length) - IPhdrlen - ((ntohs(pTCPHdr->hrf) & 0xf000) >> 10)),(char)(ntohs(pTCPHdr->hrf) & 0x00ff),is_send);
		//printf("\n");
		if(pn==25)
		{
			printf("\n");
...
// ???
		  ...
		  if(p_num > 0)
		  {
			  ...
			  for(i=0;i<p_num;i++)
			  {
				  p = port[por[i]];
				  for(int j=0;j<250;j++)
					  if(p->time[0] == flow_ptr->packet_arrival_time_list[j])
					  {
						  start = j;
				  	  	  break;
					  }
				  for(int j=249;j>=0;j--)
					  if(p->time[p->num-1] == flow_ptr->packet_arrival_time_list[j])
					  {
						  end = j;
				  	  	  break;
					  }
		  ...
		  if(cl0==0 && cl1==0)// && cl0==0)
		  {
			  ...
			  if (ret == 102)
			  {
				  ...
				  /*for(int i=0;i<250;i++)
					  printf("%d ",flow_ptr->packet_size_list[i]);
				  printf("\n");
				  for(int i=0;i<250;i++)
					  printf("%d ",flow_ptr->packet_flag_list[i]);
				  printf("\n");
				  for(int i=0;i<250;i++)
					  printf("%d ",flow_ptr->packet_sign_list[i]);
				  printf("\n");

				  for(int i=0;i<250;i++)
					  printf("%d ",flow_ptr->srcport[i]);
				  printf("\n");*/
